...
### 7.3 테스트
- [x] API 연동 코드 구현 완료 (실제 테스트는 API 키 필요)
- [x] 워크플로우 로직 구현 완료
- [x] 발행 예약 시간 계산 로직 구현 완료
...
### 8.2 배포 준비
- [x] 환경 변수 설정 문서화 (README.md에 포함)
- [x] Vercel 배포 가이드 작성
- [x] 프로덕션 빌드 테스트

### 8.3 문서화
- [x] README.md 작성
- [x] 사용 가이드 작성 (README.md에 포함)
...
### 9.1 AI 응답 파싱 개선 ✅ 완료
- [x] JSON 파싱 실패 문제 해결 (구분자 형식으로 변경)
- [x] 마크다운 코드 블록 제거 로직 추가
- [x] 플레이스홀더 시스템 도입 (`{{PRODUCT_LINK_N}}`, `{{PRODUCT_IMAGE_N}}`)
- [x] Claude/Gemini 파서 통일

### 9.2 프롬프트 개선 ✅ 완료
- [x] JSON 형식 → 구분자 형식으로 변경 (`---TITLE---`, `---CONTENT---` 등)
- [x] 평점/리뷰 정보 제거
- [x] "X개 상품" 표시 제거
- [x] 상품 HTML 형식에서 플레이스홀더 사용

### 9.3 워크플로우 개선 ✅ 완료
- [x] 상품 테이블 HTML 자동 생성 (`orchestrator.ts`)
- [x] 상품 선별 모듈 추가 (`lib/product/selector.ts`)
- [x] 가격대별 상품 분류 기능

### 9.4 워드프레스 이미지 업로드 🔧 진행 중
- [x] 이미지 파일명 ASCII 전용으로 변경 (한글 제거)
- [x] Content-Disposition 헤더 수정
- [x] 테스트 엔드포인트 추가 (`/api/wordpress/test-full`)
- [ ] **이미지 업로드 실패 문제 해결 필요**
  - 현재 상태: 업로드 실패 시 원본 URL 사용 → 외부 이미지 표시 안 됨
  - 원인 조사 필요: 워드프레스 권한, 파일 형식, 서버 설정 등
- [ ] 쿠팡 이미지 다운로드 및 워드프레스 업로드 정상화

### 9.5 추가 테스트 엔드포인트
- [x] `/api/wordpress/test` - 연결 테스트
- [x] `/api/wordpress/test-full` - 전체 워크플로우 테스트 (AI 없이)
- [x] `/api/wordpress/media` - 이미지 업로드 API
...
| # | 이슈 | 상태 | 우선순위 |
|---|------|------|----------|
| 1 | 워드프레스 이미지 업로드 실패 | 🔧 조사 중 | High |
| 2 | 쿠팡 외부 이미지 URL 표시 안 됨 | 🔧 조사 중 | High |
